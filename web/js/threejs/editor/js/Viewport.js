var Viewport = function ( editor ) {
	...
	var onDoubleClick = function ( event ) {
		...
		if ( intersects.length > 0 && intersects[ 0 ].object === editor.selected ) {
			...
			// Si l'objet est le sol, on affiche les numéro des points
			...
			if(object.name == 'Surface') {
                ...
                var geometryText = null;
                var geometrySphere = new THREE.SphereGeometry( 5, 32, 32 ); 
                var material = new THREE.MeshBasicMaterial( {color: 0xff0000} );
                var i = 0;
                for( i = 0; i < vertices.length; i++ ) {
					// Sphere
                    spheres[i] = new THREE.Mesh( geometrySphere, material );
                    spheres[i].position.x = vertices[i].x + object.position.x;
                    spheres[i].position.y = vertices[i].y + object.position.y;
                    spheres[i].position.z = vertices[i].z + object.position.z;
                    editor.addObject(spheres[i]);
                    
                    // Texte
                    geometryText = new THREE.TextGeometry(i.toString(), textOptions);
                    texts[i] = new THREE.Mesh( geometryText, material );
                    texts[i].position.x = vertices[i].x + object.position.x;
                    texts[i].position.y = vertices[i].y + object.position.y + 25;
                    texts[i].position.z = vertices[i].z + object.position.z;
                    editor.addObject(texts[i]);
                }
                
                setTimeout(function() {
                    for( i = 0; i < spheres.length; i++ ) {
                        editor.removeObject(spheres[i]);
                    }
                    for( i = 0; i < texts.length; i++ ) {
                        editor.removeObject(texts[i]);
                    }
                }, 10000);
			} 
			else {
